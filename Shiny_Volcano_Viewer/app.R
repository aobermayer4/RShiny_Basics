...
ui <- 
  navbarPage(
    title = paste("{ Volcano Viewer }",sep = ""),
    id = "vv_tabs",
    theme = shinytheme("flatly"),
    tabPanel("Volcano Plot", # tabPanel elments need to be nested in another UI such as navbarPage() or tabsetPanel()
             sidebarLayout(
               sidebarPanel(
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("Data Input",
                            ...
                            uiOutput("CoolAnimalsRender"),
                            conditionalPanel(condition = "input.VolcanoMainPanel == '1'",
                            ...
                            # Conditional panel dependent on main panel tab selected
                            conditionalPanel(condition = "input.VolcanoMainPanel == '2'",
                                             p(),
                                             selectInput("CoolAnimals","Select a cool animal:",
                                                         choices = c("Lion","Tiger","Bear","Elephant","Penguin","Duck","Goat")),
                                             # Conditional panel dependent on select input option
                                             conditionalPanel(condition = "input.CoolAnimals == 'Elephant'",
                                                              selectInput("ElephantType","What kind of elephant?:",choices = c("African","Asian"))
                                                              ),
                                             
                                             
                                             checkboxGroupInput("CoolSeaCreatures","Select cool sea creatures:",
                                                                choices = c("Sea Star","Squid","Sponge","Whale","Crab","Squirrel","Pirate"), inline = T),
                                             # Conditional panel dependent on checkbox group input
                                             conditionalPanel(condition = "input.CoolSeaCreatures.includes('Squirrel') || input.CoolSeaCreatures.includes('Pirate')",
                                                              h4("Hey that's not a sea creature!", style = "color: red;")
                                                              ),
                                             
                                             
                                             checkboxInput("MoreAnimals","Select more animals", value = F),
                                             conditionalPanel(condition = "input.SelectMoreAnimals == true",
                                                              selectizeInput("SelectMoreAnimals","Select more animals:",choices = NULL,
                                                                             options = list(
...
server <- function(input, output, session) {
  ...
  output$CoolAnimalsRender <- renderUI({
    if (input$VolcanoMainPanel == 2) {
      selectInput("CoolAnimals_rend","Select a cool animal (Render UI):",
                  choices = c("Lion","Tiger","Bear","Elephant","Penguin","Duck","Goat"))
    }
  })
